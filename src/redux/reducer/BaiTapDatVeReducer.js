...
// Data gốc đang có 2 cột D trùng nhau 
// -> sẽ bug thuật toán do đang giả định tên row là unique !!
const stateDefault = {
...
const BaiTapDatVeReducer = (state = stateDefault, action) => {
  ...
  switch (action.type) {
    ...
    // Phải bọc case lại để biến let nằm trong scope -> ko tự format
    // Khi đó có thể ĐN lại biến let được
    // Tuy nhiên ở đây đang trùng lặp code
    case HUY_GHE: {
      console.log('mã ghế hủy:', action.data);
      let index = danhSachGheDangDat_cheat.findIndex(
        (gheDangDat) => gheDangDat === action.data
      );
      console.log('index:', index);
